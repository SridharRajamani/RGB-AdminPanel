...
import { exportMemberListToPDF, exportMemberListToExcel } from '../../../utils/exportUtils';
...
const MemberManagement = ({ isSidebarCollapsed: propSidebarCollapsed = false, isSidebarVisible = true }) => {
  ...
  const handleExportMembers = (format = 'pdf') => {
    // Debug logging
    console.log('handleExportMembers called with:', format, typeof format);

    // Handle case where format might be an event object
    let exportFormat = 'pdf';
    if (typeof format === 'string') {
      exportFormat = format;
    } else if (format && format.target) {
      // This is likely an event object, default to PDF
      exportFormat = 'pdf';
    } else {
    ...
    const closeLoading = showAlert({
      type: 'loading',
      title: `Generating ${exportFormat.toUpperCase()} Export...`,
      message: 'Please wait while we prepare your member directory.',
      autoClose: false
    });
    ...
    try {
      let result;
      if (exportFormat === 'pdf') {
        result = exportMemberListToPDF(filteredMembers, filters);
      } else if (exportFormat === 'excel') {
        result = exportMemberListToExcel(filteredMembers, filters);
      }
      ...
      if (result.success) {
        showAlert({
          ...
          message: `Your member directory has been exported as ${result.fileName}`,
          duration: 4000
        });
      } else {
        showAlert({
          type: 'error',
          title: 'Export Failed',
          message: result.error || 'Failed to generate export file.',
          duration: 5000
        });
      }
    } catch (error) {
